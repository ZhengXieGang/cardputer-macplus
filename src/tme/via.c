...
static portMUX_TYPE kbdQueueMux = portMUX_INITIALIZER_UNLOCKED;
static uint8_t kbdResponse = 0x7B; // null key (no key pressed)
static uint8_t kbdLastCmd = 0;
static uint32_t kbdPushCount = 0;
static uint32_t kbdPopCount = 0;
static uint32_t kbdDropCount = 0;
...
static uint8_t kbdDequeue(void) {
    uint8_t val = 0x7B;
    portENTER_CRITICAL(&kbdQueueMux);
    if (kbdQueueHead != kbdQueueTail) {
        val = kbdQueue[kbdQueueTail];
...
void viaDebugPrint(void) {
	...
	MACPLUS_LOG("VIAHW: inb=0x%02X ddrb=0x%02X outb=0x%02X ifr=0x%02X "
		...
		(unsigned long)kbdDropCount, kbdLastCmd, kbdResponse);
}
