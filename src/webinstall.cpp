...
<div class="actions"><label class="pick"><input type="file" name="installimg" accept=".img,.dsk,.image,.dc42"><span data-t="choose">Choose Image</span></label><span class="filename" data-t="none">No file selected</span><button class="send" type="submit" data-t="upload">Upload</button></div></form>
<section class="monitor" aria-live="polite"><div class="monitor-head"><b data-t="status">Transfer Status</b><output id="percent">0%</output></div><div class="bar" id="bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span id="fill"></span></div><p class="message" id="message"></p></section>
    ...
    const char *lines[] = {
        ...
        accessPointReady ? "[OK] AP MacPlus-Install"
                         : (accessPointAttempted ? "[FAIL] AP START ERROR"
                                                 : "[RUN] STARTING AP"),
        accessPointReady ? "[OK] URL 192.168.4.1" : "URL UNAVAILABLE",
        status != nullptr ? status : "[OK] READY FOR UPLOAD",
        "HOLD ANY KEY 2S TO EXIT",
    };
    dispShowMessage(lines, 5);
}
...
static void showUploadProgress(UploadProgressStage stage,
    ...
    const int bucket = static_cast<int>(percent / 10U);
    if (stage != lastConsoleStage || bucket != lastConsoleBucket) {
        lastConsoleStage = stage;
        lastConsoleBucket = bucket;
        printf("WEB: %s %s %lu%% (%lu/%lu bytes)\n",
               uploadTargetName(uploadTarget), stageName,
...
static void handleRawUpload(UploadTarget target) {
    ...
    if (upload.status == RAW_START) {
        ...
        if (!uploadError && uploadTarget == target &&
            ...
            if (uploadIsDc42 && inputPosition < sizeof(dc42Header)) {
                ...
                if (inputPosition == sizeof(dc42Header) &&
                    ...
                    printf("WEB: invalid Disk Copy 4.2 header\n");
                    uploadError = true;
                }
            }
        ...
        if (complete && !uploadedImageSignatureIsValid(target)) {
            printf("WEB: invalid %s image signature\n",
        ...
        printf("WEB: upload end, %lu bytes, %s\n",
               static_cast<unsigned long>(uploadOffset),
...
void webInstallRun() {
    ...
    printf("WEB: open AP 'MacPlus-Install' (no password), then "
           "http://192.168.4.1\n");
    ...
    while (true) {
        ...
        if (now - lastKeyPollMs >= 20U) {
            ...
            const bool anyKey = cardputerInputAnyKeyPressed() ||
                                digitalRead(MOUSE_BUTTON_PIN) == LOW;
            if (!exitArmed) {
