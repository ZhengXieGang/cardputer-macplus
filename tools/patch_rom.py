...
ROM_SIZE = 128 * 1024
ROM_PLUS_V3_SONY_DRIVER = 0x17D30
SONY_PV_ADDRESS = 0x00C00069

# GPL-2.0-or-later replacement .Sony driver derived from evansm7/umac. The
# stub redirects Open/Prime/Control/Status to one byte store at
# SONY_PV_ADDRESS; src/tme/softfloppy.c implements the block-device calls.
# For accRun control code 65, a positive backend result makes the 68K stub call
# _PostEvent(diskEvent) and then disable periodic callbacks. This follows the
# mature Basilisk II mount path without constructing event-queue nodes in host
# code or writing into an unallocated fixed address in Macintosh RAM.
SONY_DRIVER = bytes((
    ,
    ...
    0x4E, 0x75, 0x4E, 0x71, 0x00, 0x00, 0x00, 0x00,
))
...
def patch_plus_v3(rom, ram_size):
    ...
    # Replace the hardware-timing .Sony driver with the compact block-driver
    # stub.  Its final longword is the paravirtual callback address.
    rom[ROM_PLUS_V3_SONY_DRIVER:
        ROM_PLUS_V3_SONY_DRIVER + len(SONY_DRIVER)] = SONY_DRIVER
    wr32(rom, ROM_PLUS_V3_SONY_DRIVER + len(SONY_DRIVER) - 4,
         SONY_PV_ADDRESS)
